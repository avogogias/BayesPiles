...
function Matrix(id, run_id, matrix, score, iteration) {
    ...
    for(var i=0 ; i<matrix.length; i++){
        for(var j=0 ; j<matrix[i].length; j++){
			if (i!=j)
			{
				this.nodeValues[j] = this.nodeValues[j] + matrix[i][j]/(matrix.length-1);
				this.nodeSkeletonValues[j] = this.nodeSkeletonValues[j] + matrix[i][j]/(matrix.length-1) + matrix[j][i]/(matrix.length-1); 
