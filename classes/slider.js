   ...
   //TV	.text("All Piled")
    ...
    this.g.append("text")
        ...
        .attr("y", this.height - this.BOTTOM + 20)
			.text("Create piles") //TV based on similarity between neighboring matrices
    //TV    .text("All Matrices (" + matrices.length + ")")
        .attr("class", "sliderLabel")

			

/* TV: Label buttons to change the piling up mode: Clustered or Sequential
    ...
    this.scoreBasedBtn = this.g.append("text")
        .attr("x", this.LEFT -10)
        .attr("y", this.height - this.BOTTOM + 65)
        .text("3. Score-based")
        .attr("class", "sliderLabel")
        .on('click',function(d){
            setPilingMethod('scoreBased')
            _this.sequentialBtn.style('fill', '#999')
            _this.clusteredBtn.style('fill', '#999')
			_this.scoreBasedBtn.style('fill', '#000')
        })
		
    //TV this.sequentialBtn.style('fill', '#999')
	this.clusteredBtn.style('fill', '#999')
	this.scoreBasedBtn.style('fill', '#999')
//*/
